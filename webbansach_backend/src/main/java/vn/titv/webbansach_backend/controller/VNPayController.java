...
import vn.titv.webbansach_backend.config.VNPayConfig;
...
//        vnp_Params.put("vnp_BankCode", "NCB");
    ...
    @GetMapping("/payment_info2")
    public ResponseEntity<?>  transaction(
        @RequestParam(value = "vnp_Amount") String amount,
        @RequestParam(value = "vnp_BankCode") String bankCode,
        @RequestParam(value = "vnp_OrderInfo") String oder,
        @RequestParam(value = "vnp_ResponseCode") String responseCode
    ){
        if(responseCode.equals("00")){
            return ResponseEntity.ok().body(new ThongBao("thanh cong" , "mat tien"));
        }else {
            return  ResponseEntity.badRequest().body(new ThongBao("that bai","thu lai"));
        }
    }
}